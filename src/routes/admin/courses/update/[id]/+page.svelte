<script lang="ts">
	...
	let selectedStudents: string[] = $state(data.studentsSelected);
	let selectedTeachers: string[] = $state(data.teachersSelected);
	let selectedSupervisors: string[] = $state(data.supervisorsSelected);
...
<div class="container mx-auto max-w-4xl py-8">
	<div class="mb-6">
		...
		<p class="text-muted-foreground">Edit the details of this course.</p>
	</div>
	<form
		method="POST"
		use:enhance={() => {
				...
				<CardContent class="space-y-4">
					<div class="space-y-2">
						...
						<MultiSelect
							...
							options={data.users.map((u: any) => ({ value: u.id, label: u.username }))}
